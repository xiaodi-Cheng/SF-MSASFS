...
f_bias = [0,0,0,0,0,-330,10];

if initial_flag==0
     %if func_num==1 fhd=str2func('sphere_func'); %[-100,100]
    if func_num==1 fhd=str2func('Ellipsoid'); %[-5.12,5.12]                   Unimodal
    elseif func_num==2 fhd=str2func('Rosenbrock_Pro'); %[-2.048,2.048]        Multimadal with narrow valley
    elseif func_num==3 fhd=str2func('Ackley_Pro'); %[-32.768,32.768]          Multimodal , Local optimas¡¯s number is huge ;
    elseif func_num==4 fhd=str2func('Griewank_Pro'); %[-600,600]              Multimodal  , Local optimas¡¯s number is huge ; 
    elseif func_num==5 fhd=str2func('Rastrigin');
    elseif func_num==6 fhd=str2func('rastrigin_rot_func'); %[-5,5]  CEC2005 10   Very complicated multimodal ; 
    elseif func_num==7 fhd=str2func('hybrid_rot_func2_narrow'); %[-5,5] CEC2005 19  Very complicated multimodal ; 

    
    elseif func_num==8 fhd=str2func('hybrid_rot_func1'); %[-5,5]   CEC2005 16    Very complicated multimodal ;
        
    %elseif func_num==8 fhd=str2func('high_cond_elliptic_rot_func'); %[-100,100] 3
    elseif func_num==9 fhd=str2func('rosenbrock_func'); %[-100,100] 6
    elseif func_num==10 fhd=str2func('griewank_rot_func'); %[-600,600] 7
    elseif func_num==11 fhd=str2func('ackley_rot_func'); %[-32,32] 8
    elseif func_num==12 fhd=str2func('rastrigin_func'); %[-5,5] 9 
    elseif func_num==13 fhd=str2func('weierstrass_rot'); %[-0.5,0.5] 11
    elseif func_num==14 fhd=str2func('EF8F2_func'); %[-3,1]  13
    elseif func_num==15 fhd=str2func('hybrid_func1'); %[-5,5] 15
    elseif func_num==16 fhd=str2func('hybrid_rot_func3_noncont'); %[-5,5] 23
...
%Ellipsoid Problem
    function  fit = Ellipsoid(x)
        [ps,D]=size(x);
        x_sum=zeros(ps,1);
        for i=1:ps
            for j=1:D
                x_sum(i)=x_sum(i)+j*x(i,j)^2;
            end
        end
        fit=x_sum;
        
%Rosenbrock Problem
        function fit = Rosenbrock_Pro(x)
            [ps,D]=size(x);
            fit=sum(100.*(x(:,1:D-1).^2-x(:,2:D)).^2+(x(:,1:D-1)-1).^2,2);
            
 %Ackley Problem
            function f = Ackley_Pro(x)
                 [ps,D]=size(x);
                 f=sum(x.^2,2);
                 f=20-20.*exp(-0.2.*sqrt(f./D))-exp(sum(cos(2.*pi.*x),2)./D)+exp(1);
                 
  %Griewank Problem
                function f = Griewank_Pro(x)
                    [ps,D]=size(x);
                    f=1;
                    for i=1:D
                        f=f.*cos(x(:,i)./sqrt(i));
         ...
         function f = Rastrigin(x)
              [ps,D]=size(x);
               sumx = zeros(ps,1);
                for i = 1:ps
                     for j = 1 :D
        	            sumx(i) = sumx(i) + (x(i,j))^2 - 10*cos(2*pi*x(i,j));
                     end
                    f(i) = 10*D + sumx(i); 
                end
       f = 10*D + sumx; 
